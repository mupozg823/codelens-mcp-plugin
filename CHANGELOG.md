...
- **CI clippy break on the Rust 1.97 toolchain bump (`clippy::manual_filter`)** — `cargo clippy --workspace -- -D warnings` (all five feature variants) started failing once CI's stable toolchain rolled to 1.97, whose `manual_filter` lint flags `configured_rerank_blend`'s `.and_then(|v| if (0.0..=1.0).contains(&v) { Some(v) } else { None })` range clamp. Rewritten to the equivalent `.filter(|v| (0.0..=1.0).contains(v))` (behavior identical: keep the parsed blend only when in `[0.0, 1.0]`, else fall back to `0.75`). Independent of any feature change; surfaced by the next PR's fresh build.
...
- **Watcher failure is observable (P4.1)** — `FileWatcher::start` failure at daemon/project-activation no longer degrades silently: the error is warn-logged, stored on the runtime context, and surfaced as a `watcher_unavailable` warning in `prepare_harness_session` ("index will NOT auto-update on edits" + `refresh_symbol_index` remediation, `restart_recommended: true`). Intentional no-watcher constructions (one-shot CLI) stay warning-free.
...
- **Extension-less well-known files actually index (P2.2 follow-up)** — `analyze_file` used `file.extension()?` and silently dropped `Makefile`/`Dockerfile`/`Containerfile` after candidate collection had already accepted them via the `language_for_path` filename fallback (caught by post-deploy e2e: F#/Vim extracted, Make/Dockerfile missing). Both the batch and incremental indexing paths now key extension-less files by lowercased file name, gated on the language registry so arbitrary extension-less files don't leak into the `language` column. Regression test indexes a Makefile and asserts the `build` target is extracted.
