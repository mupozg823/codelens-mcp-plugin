...
# the machine has ONE consumption daemon on :7839/:7838 that every project
...
# wrapper targets a SEPARATE dev daemon (label dev.codelens.mcp-dev, ports
# 7739/7736, binary codelens-mcp-http-dev) so dev rebuilds are isolated —
# blast radius shrinks from "all projects" to "this repo only".
#
# codelens-mcp-plugin/.mcp.json points at :7739, so this repo's own Claude
# sessions dogfood the working-tree build; all other projects stay on the
# untouched :7839 consumption daemon.
#
...
#     --readonly-port 7739 --mutation-port 7736 \
