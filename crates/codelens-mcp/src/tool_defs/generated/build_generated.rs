...
pub fn session_tools(
    ...
    vec![
        ...
        Tool::new(
            ...
            json!({"type":"object","properties":{"project":{"type":"string","description":"Optional project name or path"},"profile":{"type":"string","enum":["planner-readonly","builder-minimal","reviewer-graph","refactor-full","ci-audit"]},"preset":{"type":"string","enum":["minimal","balanced","full"]},"token_budget":{"type":"integer","description":"Optional explicit token budget override after activation"},"file_path":{"type":"string","description":"Optional file path for language-specific capability checks"},"detail":{"type":"string","enum":["compact","full"],"description":"compact returns the harness preflight essentials only; full also includes the heavier config snapshot"},"host_context":{"type":"string","enum":["claude-code","codex","cursor","cline","windsurf","vscode","jetbrains","api-agent"],"description":"Optional host/runtime hint used to compile advisory bootstrap routing without changing the active tool surface"},"task_overlay":{"type":"string","enum":["planning","editing","review","onboarding","batch-analysis","interactive"],"description":"Optional task-mode hint used to compile advisory bootstrap routing without changing the active tool surface"},"agent_role":{"type":"string","enum":["main","subagent"],"description":"Optional caller role hint used to compile advisory bootstrap routing for an orchestrating main session versus a delegated worker without changing the active tool surface"},"available_mcp_servers":{"type":"array","items":{"type":"string"},"description":"Optional host-observed MCP server ids/names. Used only for adaptive routing notes; values are not treated as trusted capabilities."},"available_mcp_tools":{"type":"array","items":{"type":"string"},"description":"Optional host-observed MCP tool ids/names. Used only for adaptive routing notes; values are not treated as trusted capabilities."},"skill_roots":{"type":"array","items":{"type":"string"},"description":"Optional host-observed skill root directories. Codex skill hints scan metadata from these roots instead of assuming default local folders."},"memory_roots":{"type":"array","items":{"type":"string"},"description":"Optional host-observed memory root directories used to report root-aware memory routing context."},"host_setting_keys":{"type":"array","items":{"type":"string"},"description":"Optional names of host setting keys observed by the client. Do not send secret values."},"harness_profile":{"type":"string","description":"Optional host harness/profile name observed by the client, separate from CodeLens profile."},"task":{"type":"string","description":"Optional user task summary used to compile Codex skill hints without loading full skill bodies"},"objective":{"type":"string","description":"Alias for task when the host carries the requested work under an objective field"},"preferred_entrypoints":{"type":"array","items":{"type":"string"},"description":"Optional ordered entrypoints so the server can report which are immediately visible. Accepts canonical tool names or mcp__codelens__-prefixed host names."},"known_tool_schema_fingerprint":{"type":"string","description":"Optional client-observed tool_schema_fingerprint. If it differs from the active surface, prepare_harness_session emits a tool_schema_cache_stale warning with refresh guidance."},"auto_refresh_stale":{"type":"boolean","description":"When true (default), bootstrap auto-refreshes a stale symbol index before reporting capabilities"},"auto_refresh_stale_threshold":{"type":"integer","description":"Optional maximum stale file count eligible for automatic refresh during bootstrap; omitted means no cap"}}}),
