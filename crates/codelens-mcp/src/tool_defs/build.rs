...
fn build_tools() -> Vec<Tool> {
    ...
    {
        tools.push(Tool::new("semantic_search", "[CodeLens:Symbol] Hybrid/path-aware natural language code search. Semantic retrieval is a supporting signal, not the sole ranking source.", json!({"required":["query"],"type":"object","properties":{"query":{"type":"string","description":"Natural language search query"},"path_hint":{"type":"string","description":"Optional file or directory prefix to prefer in ranking"},"max_results":{"type":"integer","description":"Max results (default 20)"}}})).with_output_schema(semantic_search_output_schema()).with_annotations(ro_p.clone()));
