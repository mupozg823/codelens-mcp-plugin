...
fn bridge_nl_to_code_vocabulary(query: &str, project_bridges: &[(String, String)]) -> String {
    ...
    static GENERIC_BRIDGES: &[(&str, &str)] = &[
        ...
        (
            ...
            "find_callers extract_calls call_graph",
        ),
        ("who calls", "callers"),
        ("rename a variable", "rename_symbol rename"),
        ("rename a function", "rename_symbol rename"),
        ("search code", "search"),
        ("symbol by name", "find_symbol find_symbols_by_name"),
        ("embedding vectors", "index_from_project embedding index"),
        (
            "embedding index",
            "index_from_project index_embeddings_handler",
        ),
        (
            "near-duplicate",
            "find_duplicates find_similar_code duplicate",
        ),
        (
            "similar code",
            "find_similar_code find_similar_code_handler embeddings",
        ),
        ("by its purpose", "classify_symbol classify purpose"),
        (
            "what kind of symbol",
            "classify_symbol classify_symbol_handler classify",
        ),
        ("first load", "onboard_project project structure key files"),
        (
            "recently accessed",
            "record_file_access recent_file_paths recency",
        ),
        (
            "preflight verification",
            "is_refactor_gated_mutation_tool mutation_gate",
        ),
        (
            "success response",
            "build_success_response build_suggested_next_calls",
        ),
        ("current timestamp", "now_ms timestamp_ms"),
        (
            "mutation readiness",
            "verify_change_readiness evaluate_mutation_gate",
        ),
        ("workflowfirst", "ToolProfile WorkflowFirst workflow-first"),
