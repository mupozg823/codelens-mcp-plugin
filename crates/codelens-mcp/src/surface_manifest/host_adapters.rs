...
#[path = "host_adapters/catalog.rs"]
mod catalog;
#[path = "host_adapters/overlays.rs"]
mod overlays;

use catalog::raw_host_adapter_bundle;

pub(super) fn build_host_adapters() -> Value {
    build_host_adapters_for_project(None)
}

pub(super) fn build_host_adapters_for_project(project_root: Option<&Path>) -> Value {
    json!({
        ...
        "root_causes": [
            {"code": "memory_only_routing", "problem": "Routing decisions live in chat memory, personal habit, or repo-local folklore instead of a portable product contract.", "effect": "Other repositories repeat bootstrap overhead, skip useful audits, or misuse CodeLens on trivial point edits."},
            {"code": "host_capability_blindness", "problem": "Claude Code, Codex, Cursor, and similar hosts expose different primitives for subagents, worktrees, rules, background execution, and MCP governance.", "effect": "A one-size-fits-all harness either underuses native host strengths or leaks too much surface into the wrong execution path."},
            {"code": "substrate_orchestrator_conflation", "problem": "Shared infrastructure is asked to own host UI behavior, live agent chat, and orchestration policy at the same time.", "effect": "Control-plane complexity grows faster than measurable value."},
            {"code": "eval_free_expansion", "problem": "New routing lanes, skills, or adapters are added without ground-truth data or a merge-gating signal.", "effect": "The harness bloats while quality remains unproven."}
        ],
        "design_principles": [
        ...
        "adapter_contract": {
            "detection_inputs": ["host identity", "interactive vs background execution", "task phase (lookup, plan, review, build, eval)", "risk level (single-file vs multi-file / mutation-heavy)", "need for durable artifacts or session audit"],
            "routing_outputs": ["recommended harness mode", "recommended CodeLens profile", "preferred native config targets", "whether handoff artifacts are required", "whether analysis jobs should replace direct long reports"]
        },
        "delegate_scaffold_contract": {
            ...
            "telemetry_fields": ["delegate_hint_trigger", "delegate_target_tool", "delegate_handoff_id", "handoff_id"]
        },
...
pub(crate) fn harness_host_compat_bundle_for_project(
    ...
    Some(json!({
        ...
        "detected_host": {
            ...
            "task_stages": ["discover", "investigate", "act", "verify", "handoff"],
            "guardrails": guardrails
        }
    }))
}
