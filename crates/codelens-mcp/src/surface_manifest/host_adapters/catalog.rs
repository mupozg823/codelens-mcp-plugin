use super::overlays::{
    append_compiled_overlay_section, host_context_for_adapter, managed_host_policy_block,
    overlay_previews_for_host, primary_bootstrap_sequence_for_host,
};
use super::*;
...
pub(super) fn raw_host_adapter_bundle(host: &str) -> Option<Value> {
    let mut bundle = match host {
        "claude-code" => json!({
            ...
            "native_primitives": ["CLAUDE.md", "subagents and agent teams", "hooks", "managed-mcp.json and .mcp.json", "subagent-scoped MCP servers"],
            "preferred_codelens_use": ["bootstrap and bounded architecture review", "preflight before dispatching a builder", "planner-session audit and handoff artifact production"],
            "routing_defaults": {
            ...
            "compiler_targets": ["CLAUDE.md", ".mcp.json", "managed-mcp.json", "subagent definitions"],
            "native_files": [
                {
                    "path": ".mcp.json",
                    "format": "json",
                    "purpose": "Attach the read-only CodeLens daemon to the project by default.",
                    "template": {"mcpServers": {"codelens": {"type": "http", "url": "http://127.0.0.1:7837/mcp"}}}
                },
                {
                    "path": "CLAUDE.md",
                    "format": "markdown",
...
"#, host))
        ...
        "codex" => json!({
            ...
            "native_primitives": ["AGENTS.md", "skills", "worktrees", "shared MCP config", "CLI, app, and IDE continuity"],
            "preferred_codelens_use": ["bounded mutation after verify_change_readiness", "session-scoped builder audit", "analysis jobs for CI-facing summaries"],
            "routing_defaults": {
                "point_lookup": "native-first",
...
"#, host))
        ...
        "cursor" => json!({
            ...
            "native_primitives": [".cursor/rules", "AGENTS.md", "custom modes", "background agents", "mcp.json"],
            "preferred_codelens_use": ["architecture review and diff-aware signoff", "analysis jobs for background-agent queues", "minimal surface exposure through mode- or rule-specific routing"],
            "routing_defaults": {
            ...
            "compiler_targets": [".cursor/rules", "AGENTS.md", ".cursor/mcp.json", "background-agent environment.json"],
            "native_files": [
                {
...
"#, host)
        ...
        "cline" => json!({
            ...
            "native_primitives": ["interactive permissioned terminal execution", "browser loop", "workspace checkpoints", "MCP integrations"],
            "preferred_codelens_use": ["review-heavy exploration before write passes", "session audit and handoff artifacts when a change must cross sessions"],
            "routing_defaults": {
            ...
            "compiler_targets": ["mcp_servers.json", ".clinerules", "repo instructions"],
            "native_files": [
                {
                    "path": "mcp_servers.json",
                    "format": "json",
                    "purpose": "Attach CodeLens to Cline with an explicit project-local server entry.",
                    "template": {"codelens": {"type": "http", "url": "http://127.0.0.1:7837/mcp"}}
                },
                {
                    "path": ".clinerules",
                    "format": "markdown",
...
"#, host))
        ...
        "windsurf" => json!({
            ...
            "native_primitives": ["global MCP config", "foreground agent loop", "workspace-local editing", "100-tool cap across MCP servers"],
            "preferred_codelens_use": ["bounded builder execution under a small visible surface", "compressed planning when the task escapes single-file scope"],
            "routing_defaults": {
                "foreground_lookup": "native-first",
            ...
            "compiler_targets": ["~/.codeium/windsurf/mcp_config.json"],
            "native_files": [
                {
