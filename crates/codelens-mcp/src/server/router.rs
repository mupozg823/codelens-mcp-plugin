        ...
        "initialize" => {
            ...
            Some(JsonRpcResponse::result(
                ...
                json!({
                    ...
                    "instructions": "FIRST CALL of every session: prepare_harness_session with project=<absolute path of YOUR current workspace> — a shared CodeLens daemon otherwise targets its own default project and every read silently comes from the wrong repo. (Hosts can bind automatically via the x-codelens-project HTTP header or an initialize `project` param; if a response carries a `project_binding` hint, the session is still unbound.) Project binding reselects the project-local symbol index, graph/LSP runtime, analysis cache, and SCIP precise index; switching projects auto-builds the symbol index on first use. After binding, prefer problem-first workflow entrypoints such as explore_codebase, review_architecture, plan_safe_refactor, trace_request_path, review_changes, and cleanup_duplicate_logic before expanding raw symbols or graph data. Keep the visible context bounded, and use get_analysis_section or analysis resources only when you need one section in more detail. For longer reports, start_analysis_job and poll with get_analysis_job."
                }),
            ))
        }
