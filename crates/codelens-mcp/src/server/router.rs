...
use crate::resources::tool_listing::build_tools_list_payload;
...
pub(crate) fn handle_request(state: &AppState, request: JsonRpcRequest) -> Option<JsonRpcResponse> {
    ...
    match request.method.as_str() {
        ...
        "tools/list" => {
            let listing = build_tools_list_payload(state, request.params.as_ref());
            if listing.deferred_expansion_requested {
            ...
            state.metrics().record_call_with_tokens(
                ...
                0,
                true,
                listing.token_estimate,
                listing.active_surface_label,
                false,
                None,
            );
            Some(JsonRpcResponse::result(request.id, listing.payload))
        }
