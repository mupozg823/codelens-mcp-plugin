...
pub(super) fn inspect_host_file(path: &Path, format: &str, template: Option<&Value>) -> String {
    match format {
        "json" => template.map_or_else(|| {
                format!(
                    "- {} [json]: present but manual review required",
                    path.display()
                )
            }, |template| inspect_json_config_entry(path, template)),
        "toml" => template
            ...
            }, |template| inspect_toml_section(path, template)),
        "markdown" | "mdc" => template
            ...
            }, |template| inspect_text_policy_file(path, template, format)),
        other => {
            if path.exists() {
...
pub(super) fn inspect_host_file_json(path: &Path, format: &str, template: Option<&Value>) -> Value {
    match format {
        "json" => template.map_or_else(|| {
            ...
            }, |template| inspect_json_config_entry_json(path, template)),
        "toml" => template
            .and_then(Value::as_str).map_or_else(|| {
                json!({
                    "path": path.display().to_string(),
            ...
            }, |template| inspect_toml_section_json(path, template)),
        "markdown" | "mdc" => template
            .and_then(Value::as_str).map_or_else(|| {
                json!({
                    "path": path.display().to_string(),
            ...
            }, |template| inspect_text_policy_file_json(path, template, format)),
        other => {
            if path.exists() {
                json!({
                    "path": path.display().to_string(),
