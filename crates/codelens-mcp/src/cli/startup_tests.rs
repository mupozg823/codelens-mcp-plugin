...
fn render_attach_instructions_rejects_unknown_hosts() {
    let error = render_attach_instructions(Some("openhands")).expect_err("unknown host must fail");
...
fn doctor_report_detects_machine_attachment_and_customized_policy_file() {
    ...
    let report = render_doctor_report("doctor", &["cursor"], &home, &cwd).expect("doctor report");
...
fn doctor_report_marks_missing_codex_files() {
    ...
    let report = render_doctor_report("doctor", &["codex"], &home, &cwd).expect("doctor report");
...
fn run_doctor_command_renders_json_report() {
    ...
    let payload: serde_json::Value = serde_json::from_str(&rendered).expect("valid doctor json");
...
fn run_status_command_renders_status_alias_in_text_and_json() {
    ...
    let payload: serde_json::Value = serde_json::from_str(&rendered).expect("valid status json");
