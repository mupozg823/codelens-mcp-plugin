use super::{
    inspect::{inspect_host_file, inspect_host_file_json},
    render::{append_host_adapter_common_metadata, host_adapter_common_metadata_json},
    resolve::{
...
pub(super) fn parse_doctor_hosts(args: &[String]) -> Result<Vec<&'static str>> {
    let filtered = args[2..]
        .iter()
...
pub(super) fn doctor_report_json(
    command: &str,
    hosts: &[&str],
    home: &Path,
    cwd: &Path,
