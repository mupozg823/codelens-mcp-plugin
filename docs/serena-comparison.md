...
- **P4 (internal architecture hygiene)**: split large mixed-responsibility CodeLens modules only where the split removes real ownership friction. The `tools/workflows.rs` duplicate-cleanup filter split is complete, `session_metrics_payload.rs` is now split into parent orchestration plus session-field, derived-KPI, and token-bill submodules, and the telemetry analyzer rollout parsing is separated from loader orchestration. The next candidate is `project.rs` detection helpers after a fresh CodeLens blast-radius pass. Do not start with `dispatch/response.rs`; it is large but the current boundary has no cycle hit.
