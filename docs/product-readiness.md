...
| Agent productivity | `docs/productivity-study.md`, `benchmarks/harness/productivity_study_*.py`, immutable per-run manifests outside the repository |
...
| Productivity | Agent productivity claims must be backed by paired completed tasks, native-agent usage, rework, hidden acceptance or blind review, and resource evidence. | `productivity-study-v1` now supplies historical-base worktree isolation, baseline/naive/routed conditions, Codex/Claude stream adapters, immutable policy SHA, raw-stream deletion, hidden diff/test grading, an isolated HTTP daemon with structured metrics plus CPU/RSS sampling, and warm-only Pareto gates. The current pilot task pack contains 48 planned runs; it has no completed cohort result yet. The existing proof loop is telemetry provenance and runtime-health support only. | Execute the 48-run pilot with pinned models, finish blind review, close metric coverage gaps, then run the 180-run cohort before changing routing defaults or public productivity claims. |
