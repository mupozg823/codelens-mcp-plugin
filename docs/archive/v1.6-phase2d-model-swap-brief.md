...
**Checkpoint 1 prerequisites — must be satisfied before the session starts**:

These are the practical blockers a follow-up session needs to resolve before Task 1.1 can run. They are _not_ blocking this brief from being merged, but they define the minimum setup a real Checkpoint 1 session needs.

1. **Network / HuggingFace access for ONNX INT8 artefact download** — the three short-list candidates need INT8 ONNX exports pulled from HuggingFace (`BAAI/bge-small-en-v1.5`, `jinaai/jina-embeddings-v2-base-code`, `thenlper/gte-small`). Any session without egress to `huggingface.co` cannot start Task 1.1. The artefacts must be pinned by SHA256 on download so future reruns stay reproducible — mirror the pinned digests into `benchmarks/phase2d-artefacts.json` (new file) alongside `artefact_url`, `model_name`, and `expected_embedding_dim`.
2. **Model loader refactor scope** — the current `crates/codelens-engine/src/embedding/mod.rs` loader path is hard-coded to the bundled CodeSearchNet artefact. Task 1.2's `CODELENS_EMBED_MODEL_OVERRIDE=<path>` hack needs the loader to (a) accept an override path, (b) read the tokenizer from the same directory, (c) route the embedding dim to the correct vector-store config. This is probably ~100–150 lines of Rust across `embedding/mod.rs` + wherever `resolve_runtime_model_dir` lives. The refactor is throwaway — it lives on a measurement branch and is not merged.
3. **Tokenizer compatibility** — every short-list candidate uses a different tokenizer vocabulary. The temporary loader needs to load `tokenizer.json` from the override directory, not the bundled one. Forgetting this silently produces garbage embeddings that pass the ONNX forward pass but fail retrieval. **This is the single most likely cause of a false zero result** — add a sanity-check unit test that embeds `"parse json body"` and confirms the vector dim matches the candidate's declared dim.
4. **Query prefix convention** (E5 / Nomic-style) — while BGE-small / Jina code v2 / gte-small do _not_ require `query:` / `passage:` prefixes, any replacement candidate pulled from the second-pass list in §2 does. The temporary loader should expose a `prefix` field per-model so the first-pass code does not silently break when the second pass adds a prefix-using candidate.
5. **Vector dimension migration (Jina code v2 only)** — the current `embedding_store.rs` vec table is configured for 384-dim vectors. Jina code v2 is 768-dim. The temporary loader either recreates the DB with the new dim or refuses to run with a clear error. The 384-dim short-list candidates (BGE-small, gte-small) do not need this.
6. **Measurement infrastructure already in place** — `benchmarks/embedding-quality.py` + `--isolated-copy` + the 89/436-query datasets + the Phase 2g sweep harness are all ready. Task 1.3 reuses them unchanged. The only new thing per candidate is the `CODELENS_EMBED_MODEL_OVERRIDE=<path>` env var at launch time.
7. **Compute budget** — each 4-arm A/B on the 89-query self dataset takes roughly 8–15 minutes wall time (baseline + three gates + stack, with cold index rebuild per `--isolated-copy`). Three candidates × two datasets (89 + 436) × 4 arms each = **roughly 2–3 hours** of contiguous measurement. Plus ~30–60 minutes for the model loader refactor (Task 1.2). Reserve a half-day session minimum.
8. **Go / no-go threshold** — the winner must exceed `hybrid MRR > 0.586` on the 89-query self dataset _before_ spending compute on the 436-query replay or anything downstream. Enforce this in the task runner: if Task 1.3 on candidate N measures ≤ 0.586, do not run the 436-query replay for that candidate.

**If any of 1–5 cannot be resolved in a single session, Checkpoint 1 is deferred to the next session instead of partially started**. Partial Checkpoint 1 runs (e.g. one candidate measured but no tokenizer sanity check) produce results that look like data but are not — that is the exact class of failure the Phase 2 cAST PoC revert is meant to prevent from repeating.
...
Pre-Checkpoint-1 commitments filled by the maintainer (2026-04-12 session). The "Answer" column is the authoritative record; the "Default recommendation" column stays for historical comparison with the pre-filled draft.

| Decision                                                                                | Default recommendation                                                                              | Answer (2026-04-12)                                                                                                                                                                                                                                                                                                                                                                                                                            |
| --------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D1: Is Phase 2d worth attempting at all given v1.5.0 already ships a +0.014 hybrid MRR? | Yes — the NL ceiling is real                                                                        | **Yes — proceed to Checkpoint 1** (short-list measurement only). The NL hybrid MRR gap between the pure semantic path (0.522) and the identifier Acc@1 (0.800) is large enough that a better NL-capable embedding is the single cheapest ceiling-raiser available. Green-light is scoped to measurement; Checkpoint 2+ remains gated on the Checkpoint 1 outcome.                                                                              |
| D2: Is 3× current cold-start time the acceptable budget?                                | Yes                                                                                                 | **Yes, conditional**. 3× is the hard ceiling; within that, measure throughput (Task 3.4 "non-measured dimensions") on the winner and report to the maintainer before flipping any default. If the measured cost is > 2×, ship opt-in first and push the default flip to the _next_ release.                                                                                                                                                    |
| D3: Which bundle strategy (§4)?                                                         | A if winner < 50 MB, B if winner 50–200 MB                                                          | **A for short-list measurement (Checkpoint 1), decide A-vs-B only after the winner's artefact size is measured**. Checkpoint 1 itself does not ship a model — the temporary `CODELENS_EMBED_MODEL_OVERRIDE` hack in Task 1.2 loads from a local path. Bundle strategy is a Checkpoint 2 decision, not a Checkpoint 1 blocker.                                                                                                                  |
| D4: Which short-list candidate to measure first?                                        | BGE-small, Jina code v2, gte-small (all three)                                                      | **All three, in that order** (BGE-small → Jina code v2 → gte-small). Stop criterion: if BGE-small wins _and_ exceeds the v1.5 stacked baseline (hybrid MRR > 0.586 on 89-query) by more than 0.010, skip Jina / gte unless their install-size-vs-quality trade-off is materially different. If BGE-small fails, the other two still run — we need three data points to distinguish "wrong candidate class" from "whole short-list inadequate". |
| D5: External-repo choice for Checkpoint 3?                                              | `ripgrep` (mid-size Rust, search-related queries match intent)                                      | **`ripgrep`** (`github.com/BurntSushi/ripgrep`). Mid-size, well-known, search-related intent matches the kind of queries we're trying to improve. Hand-built dataset target: 20–30 queries covering regex matching, directory traversal, ignore-file handling, output formatting. Split 70/20/10 between natural_language / short_phrase / identifier to mirror the 89-query shape.                                                            |
| D6: If all three short-list candidates fail, retry with FP16 / a different short-list?  | No — park Phase 2d for v2, measured rejection is evidence too                                       | **No, hard stop**. If all three fail to exceed v1.5 stacked hybrid MRR (0.586), Phase 2d is parked and a §8.7 experiment log entry is added under the Phase 2 cAST PoC revert pattern. A second short-list would be a decision for a separate brief, not an automatic retry — the §8.1 lesson is that repeated implementation without a new hypothesis wastes more than it learns.                                                             |
| D7: Version to release under (1.6.0 vs 2.0.0)?                                          | 1.6.0 if the swap stays backwards-compatible via rollback env var; 2.0.0 if the index schema breaks | **v1.6.0** if the auto-reindex path (§5.1) handles the transition without user intervention and the rollback env var (§5.3) works across one release cycle. **v2.0.0** only if the index schema requires a migration step the user has to run (not just an automatic reindex). The default expectation is v1.6.0.                                                                                                                              |
