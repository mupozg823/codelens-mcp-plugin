...
- Roles:
  - `solo-agent`: `planner-readonly` (42), `builder-minimal` (41); mutate=`false`; one session handles both planning and implementation
- Recommended flow:
...
- Roles:
  - `planner-reviewer`: `planner-readonly` (42), `reviewer-graph` (46); mutate=`false`; bootstrap, rank context, and verify change readiness before dispatch
  - `builder-refactor`: `builder-minimal` (41), `refactor-full` (41); mutate=`true`; execute bounded edits after preflight, diagnostics, and claims
- Recommended flow:
  - `prepare_harness_session`
...
- Roles:
  - `reviewer`: `reviewer-graph` (46), `ci-audit` (46); mutate=`false`; diff-aware review, impact analysis, and audit signoff
- Recommended flow:
  - `prepare_harness_session`
...
- Recommended ports: `7837`
- Roles:
  - `analysis-runner`: `workflow-first` (42), `evaluator-compact` (42), `ci-audit` (46); mutate=`false`; start durable jobs and consume bounded sections instead of raw full reports
- Recommended flow:
  - `prepare_harness_session`
  - `start_analysis_job`
  - `get_analysis_job`
