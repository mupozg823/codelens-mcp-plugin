...
- Roles:
  - `solo-agent`: `planner-readonly` (35), `builder-minimal` (36); mutate=`false`; one session handles both planning and implementation
- Recommended flow:
...
- Roles:
  - `planner-reviewer`: `planner-readonly` (35), `reviewer-graph` (35); mutate=`false`; bootstrap, rank context, and verify change readiness before dispatch
  - `builder-refactor`: `builder-minimal` (36), `refactor-full` (49); mutate=`true`; execute bounded edits after preflight, diagnostics, and claims
- Recommended flow:
  - `prepare_harness_session`
...
- Roles:
  - `reviewer`: `reviewer-graph` (35), `ci-audit` (43); mutate=`false`; diff-aware review, impact analysis, and audit signoff
- Recommended flow:
  - `prepare_harness_session`
