...
surface 게이트(presets.rs)는 이미 존재한다. `:7838` 단일 writer에서 세션별로 선택되는 기본 `review` 프로필이 참조하는 상수
...
## 결정 확정 (2026-07-19, 사용자 승인)

3렌즈 패널(token-DX·enterprise-positioning·migration-risk) 판정을 거쳐 확정:

1. **core-20 유지, core-15 기각** (만장일치). alwaysLoad 9종 + canonical verb 5종 = 14가
   공표된 계약 바닥(v1.13.34 CHANGELOG + codelens-first 훅/harness.md 안내)이라 15는 구조적으로
   불가하고, claude 프로필 eager는 이미 5-tool/1.76Ktok라 추가 축소의 실절감이 0이다.
   `semantic_search`는 raw 도구로 미노출 유지(`search(mode=semantic)` façade + tools/call로 접근).
   재검토 조건: 비운영자(외부) 텔레메트리에서 재량 6종 중 진짜 0-사용 도구가 확인되고
   그 도구가 라우팅 오버레이·mutation 게이트·훅 어디에서도 참조되지 않을 때만.
2. **2단계 4패밀리(memory 도구군·agent coordination·operator 대시보드·RBAC principals)는
   영구 미노출** (2:1 — 삭제 소수의견은 v2.0 포지셔닝 명료성). 정의·dispatch·tools/call 경로는
   유지하고 모든 기본 preset 표면에서 제거한다. 실제 삭제는 v2.0 릴리스 트레인에서 외부 수요
   신호를 본 뒤 별도 결정(semver major + 마이그레이션 노트 전제).
3. 실행: 1단계는 2026-07-13 적용 완료, 2단계(미노출)는 2026-07-19 착수.
