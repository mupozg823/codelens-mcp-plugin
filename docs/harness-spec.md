...
- Roles:
  - `planner-reviewer`: `planner-readonly` (36), `reviewer-graph` (40); mutate=`false`; collect structure, diagnostics, and readiness evidence before dispatch
  - `builder-refactor`: `builder-minimal` (35), `refactor-full` (35); mutate=`true`; perform bounded mutation only after preflight, diagnostics, and coordination
...
- Roles:
  - `reviewer`: `reviewer-graph` (40), `ci-audit` (40); mutate=`false`; perform diff-aware review, signoff, and audit validation without content mutation
...
- Roles:
  - `analysis-runner`: `workflow-first` (36), `evaluator-compact` (36), `ci-audit` (40); mutate=`false`; queue durable read-side jobs and consume bounded sections
