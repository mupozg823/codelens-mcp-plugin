...
- Roles:
  - `planner-reviewer`: `planner-readonly` (39), `reviewer-graph` (43); mutate=`false`; collect structure, diagnostics, and readiness evidence before dispatch
  - `builder-refactor`: `builder-minimal` (38), `refactor-full` (38); mutate=`true`; perform bounded mutation only after preflight, diagnostics, and coordination
...
- Roles:
  - `reviewer`: `reviewer-graph` (43), `ci-audit` (43); mutate=`false`; perform diff-aware review, signoff, and audit validation without content mutation
...
- Roles:
  - `analysis-runner`: `workflow-first` (39), `evaluator-compact` (39), `ci-audit` (43); mutate=`false`; queue durable read-side jobs and consume bounded sections
