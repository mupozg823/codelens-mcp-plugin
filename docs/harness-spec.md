...
- Roles:
  - `planner-reviewer`: `planner-readonly` (34), `reviewer-graph` (38); mutate=`false`; collect structure, diagnostics, and readiness evidence before dispatch
  - `builder-refactor`: `builder-minimal` (39), `refactor-full` (54); mutate=`true`; perform bounded mutation only after preflight, diagnostics, and coordination
...
- Roles:
  - `reviewer`: `reviewer-graph` (38), `ci-audit` (46); mutate=`false`; perform diff-aware review, signoff, and audit validation without content mutation
...
- Roles:
  - `analysis-runner`: `workflow-first` (22), `evaluator-compact` (14), `ci-audit` (46); mutate=`false`; queue durable read-side jobs and consume bounded sections
