...
- Roles:
  - `planner-reviewer`: `planner-readonly` (42), `reviewer-graph` (46); mutate=`false`; collect structure, diagnostics, and readiness evidence before dispatch
  - `builder-refactor`: `builder-minimal` (41), `refactor-full` (41); mutate=`true`; perform bounded mutation only after preflight, diagnostics, and coordination
...
- Roles:
  - `reviewer`: `reviewer-graph` (46), `ci-audit` (46); mutate=`false`; perform diff-aware review, signoff, and audit validation without content mutation
...
- Roles:
  - `analysis-runner`: `workflow-first` (42), `evaluator-compact` (42), `ci-audit` (46); mutate=`false`; queue durable read-side jobs and consume bounded sections
