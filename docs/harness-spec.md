...
- Roles:
  - `planner-reviewer`: `planner-readonly` (29), `reviewer-graph` (34); mutate=`false`; collect structure, diagnostics, and readiness evidence before dispatch
  - `builder-refactor`: `builder-minimal` (29), `refactor-full` (29); mutate=`true`; perform bounded mutation only after preflight, diagnostics, and coordination
...
- Roles:
  - `reviewer`: `reviewer-graph` (34), `ci-audit` (34); mutate=`false`; perform diff-aware review, signoff, and audit validation without content mutation
...
- Roles:
  - `analysis-runner`: `workflow-first` (29), `evaluator-compact` (29), `ci-audit` (34); mutate=`false`; queue durable read-side jobs and consume bounded sections
