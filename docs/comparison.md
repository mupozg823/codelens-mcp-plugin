...
> 출처: [`tools.toml`](../crates/codelens-mcp/tools.toml) 87 entries (v1.13.32) · [oraios/serena](https://github.com/oraios/serena) **v1.5.3** 소스 감사 (2026-06-10 갱신).
...
| 선언/구현 찾기       | ❌ (P1 설계: D1)                                          | `find_declaration`, `find_implementations` (v1.3) | ❌                                    | **Serena** — LSP 정밀 navigation 루프 완결                   |
...
**2026-06-10 실측**: CodeLens mutation 도구들(`replace_symbol_body`, `insert_before/after_symbol`,
`rename_symbol`, `refactor_*`, `add_import` 등)은 **dispatch에는 남아 있으나 `tools.toml`에 없어
`tools/list` 어디에도 노출되지 않는 유령 상태**다 (`BUILDER_MINIMAL_TOOLS` 주석: "Deprecated…
will be removed in v2.0"). 에이전트가 발견·호출할 수 없으므로 비교표에서 "가용"으로 표기하지 않는다.
...
> 👻 주의: `onboard_project`, `analyze_change_request`, `orchestrate_change`도 현재 ghost
> (dispatch-only). 표에는 노출 중인 대체 경로만 기재.
...
| 심볼 단위 진단         | ❌ (P1 설계: D1)                                | `get_diagnostics_for_symbol` (v1.3)               | ❌        |
...
- "인터페이스 구현체 다 찾기" → Serena `find_implementations` (CodeLens는 P1 D1 예정)
...
dispatch에는 있으나 tools.toml에 없어 어떤 `tools/list`에도 안 보이는 도구 20종:
mutation 가족 16 (`rename_symbol`, `replace_symbol_body`, `insert_*`, `replace*`, `delete_lines`,
`create_text_file`, `add_import`, `propagate_deletions`, `refactor_*` ×4) + workflow 3
(`analyze_change_request`, `onboard_project`, `orchestrate_change`) + 기타. 해소 설계:
