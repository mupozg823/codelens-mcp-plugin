...
- Current product-readiness roadmap: [docs/plans/PLAN_product-readiness-hardening.md](plans/PLAN_product-readiness-hardening.md)
...
This section avoids hand-maintained full tool tables. The authoritative
machine-readable source is
[`docs/generated/surface-manifest.json`](generated/surface-manifest.json),
which is regenerated by `scripts/surface-manifest.py`.

### Default Public Surface

| Namespace | Tool count | Product role |
| --- | ---: | --- |
| `session` | 25 | project activation, profile/preset control, coordination, metrics, audit/export |
| `reports` | 20 | bounded reports, verifier reports, analysis jobs, compressed context |
| `symbols` | 9 | symbol lookup, ranked context, complexity, fuzzy/BM25 lookup |
| `graph` | 6 | changed files, call graph, scoped references, symbol importance |
| `filesystem` | 5 | file reads, directory/file discovery, annotations, tests |
| `memory` | 5 | project memory CRUD |
| `lsp` | 2 | diagnostics and language-server-backed references |

Default generated surface: 72 tool definitions, 52 output schemas, 30
language families, and 49 file extensions.

Semantic tools add 6 feature-gated definitions when built with
`--features semantic`; they are intentionally not part of the default
`cargo install codelens-mcp` surface.

### Surface Policy

- Prefer workflow-first tools: `explore_codebase`, `review_architecture`,
  `trace_request_path`, `review_changes`, `plan_safe_refactor`, and
  `cleanup_duplicate_logic`.
- Use report tools when a bounded answer is needed: `impact_report`,
  `module_boundary_report`, `refactor_safety_report`,
  `diff_aware_references`, and `dead_code_report`.
- Use primitive graph/symbol tools only after the workflow result narrows
  the target file, symbol, or module.
- Low-level mutation dispatch remains a compatibility/runtime concern; it
  is not the public product surface to advertise in docs.
...
| impact_report        | ~1ms graph core + report envelope | Graph cache                 |
