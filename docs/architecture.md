...
Agent productivity is a separate evidence plane, not a daemon-metrics
shortcut. [`productivity-study-v1`](productivity-study.md) starts each run from
a pinned historical worktree, runs native/naive/routed conditions against an
isolated daemon, and permits routing-policy changes only after hidden quality,
blind review, and warm paired Pareto gates pass.
