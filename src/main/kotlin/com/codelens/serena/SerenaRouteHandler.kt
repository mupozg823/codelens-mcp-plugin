...
internal class SerenaRouteHandler(
    private val project: Project,
    private val compat: SerenaCompatSymbols
) {
    private val logger = Logger.getInstance(SerenaRouteHandler::class.java)

    fun registerRoutes(httpServer: HttpServer) {
        httpServer.createContext("/status", JsonHandler(project) { _ ->
            mapOf(
                "projectRoot" to (project.basePath ?: ""),
                "pluginVersion" to SerenaCompatServer.COMPAT_PLUGIN_VERSION
            )
        })
    ...
    internal class RequestJson(private val values: Map<String, Any?>) {
