...
|---|---:|---:|---:|---:|---:|
| semantic_search | 0.046 | 4% | 5% | 6% | 231.8 |
| get_ranked_context_no_semantic | 0.041 | 3% | 4% | 5% | 31.7 |
| get_ranked_context | 0.048 | 4% | 5% | 6% | 111.8 |

## Query Type Breakdown

| Method | Query type | Count | MRR | Acc@1 | Acc@3 | Acc@5 | Avg ms |
|---|---|---:|---:|---:|---:|---:|---:|
| semantic_search | identifier | 83 | 0.096 | 10% | 10% | 10% | 129.6 |
| semantic_search | natural_language | 299 | 0.037 | 3% | 4% | 5% | 262.4 |
| semantic_search | short_phrase | 54 | 0.021 | 2% | 2% | 2% | 219.2 |
| get_ranked_context_no_semantic | identifier | 83 | 0.096 | 10% | 10% | 10% | 22.9 |
| get_ranked_context_no_semantic | natural_language | 299 | 0.032 | 2% | 4% | 4% | 34.3 |
| get_ranked_context_no_semantic | short_phrase | 54 | 0.005 | 0% | 0% | 2% | 31.1 |
| get_ranked_context | identifier | 83 | 0.096 | 10% | 10% | 10% | 23.2 |
| get_ranked_context | natural_language | 299 | 0.039 | 3% | 4% | 5% | 133.2 |
...
| semantic_search | upload image and resize to multiple dimensions | miss | is_nl_shaped_rejects_code_and_paths (crates/codelens-engine/src/embedding/mod.rs) |
...
| semantic_search | handle file drag and drop in a React component | miss | partition_handles_renames (crates/codelens-engine/src/vfs.rs) |
...
| semantic_search | generate OTP for two-factor authentication | miss | generate_python_import (crates/codelens-engine/src/auto_import.rs) |
...
| semantic_search | decode a JWT token without verifying the signature | miss | build_embedding_text_with_signature (crates/codelens-engine/src/embedding/mod.rs) |
...
| semantic_search | define a FastAPI route with path and query parameters | miss | make_project_with_symbols (crates/codelens-engine/src/search.rs) |
...
| semantic_search | authenticate user and return JWT access token | miss | search_scored_returns_raw_chunks (crates/codelens-engine/src/embedding/mod.rs) |
...
| semantic_search | parse CSV file into a list of dicts | miss | parser (scripts/artifact_maintenance.py) |
...
| semantic_search | filter Django queryset by date range | miss | decode_embedding_bytes (crates/codelens-engine/src/embedding/vec_store.rs) |
...
| semantic_search | run data validation and return a list of errors | miss | get_imports_of (crates/codelens-engine/src/db/ops.rs) |
| semantic_search | split a list into chunks of a given size | miss | with_audit_category (crates/codelens-mcp/src/protocol.rs) |
...
| semantic_search | validate a JWT token and return claims | miss | mermaid_header_and_target_node_are_first (crates/codelens-mcp/src/tools/reports/impact_reports.rs) |
...
| semantic_search | parse command-line flags using the flag package | miss | parse (crates/codelens-mcp/src/dispatch.rs) |
...
| semantic_search | use sync.WaitGroup to wait for goroutines | miss | maybe_delay (crates/codelens-mcp/src/tools/report_jobs.rs) |
...
| semantic_search | rate limit requests using a token bucket algorithm | miss | duplicate_candidate_limit (crates/codelens-engine/src/embedding/mod.rs) |
...
| semantic_search | send an email with Spring Mail and Thymeleaf template | miss | is_static_method_ident_accepts_pascal_and_rejects_snake (crates/codelens-engine/src/embedding/mod.rs) |
...
| semantic_search | implement a custom Serde deserializer for a field | miss | insert_at_line_tool (crates/codelens-mcp/src/tools/mutation.rs) |
...
| semantic_search | check if two byte slices are equal in constant time | miss | insert_if_present (crates/codelens-mcp/src/dispatch_response_support.rs) |
...
| semantic_search | serialize an ActiveRecord model to JSON with JBuilder | miss | with_tier (crates/codelens-mcp/src/protocol.rs) |
...
| semantic_search | implement a custom Devise strategy for API token auth | miss | configured_coreml_specialization_strategy_name (crates/codelens-engine/src/embedding/mod.rs) |
...
| semantic_search | CSV row parser utility | miss | parser (scripts/artifact_maintenance.py) |
...
| get_ranked_context_no_semantic | check if user has permission to access a resource | miss | has_dependency (crates/codelens-engine/src/project.rs) |
...
| get_ranked_context | connect to database using SQLAlchemy async session | miss | db (crates/codelens-engine/src/symbols/mod.rs) |
...
| get_ranked_context | send a webhook notification on model save | miss | JsonRpcNotification (crates/codelens-mcp/src/protocol.rs) |
...
| get_ranked_context | configure CORS allowed origins in FastAPI | miss | cosine_sim (models/benchmark_hard.py) |
...
| get_ranked_context | parse JSON request body into a struct | miss | JsonRpcRequest (crates/codelens-mcp/src/protocol.rs) |
...
| get_ranked_context | implement OpenTelemetry trace span for a function | miss | EffortLevel (crates/codelens-mcp/src/client_profile.rs) |
...
| get_ranked_context | check if two byte slices are equal in constant time | miss | all_symbols_with_bytes (crates/codelens-engine/src/db/ops.rs) |
...
| get_ranked_context | user model serialization | miss | model (models/benchmark_full.py) |
