...
|---|---:|---:|---:|---:|---:|
| semantic_search | 0.517 | 47% | 54% | 55% | 249.2 |
| get_ranked_context_no_semantic | 0.493 | 42% | 54% | 60% | 31.9 |
| get_ranked_context | 0.569 | 49% | 63% | 65% | 111.7 |

## Query Type Breakdown

| Method | Query type | Count | MRR | Acc@1 | Acc@3 | Acc@5 | Avg ms |
|---|---|---:|---:|---:|---:|---:|---:|
| semantic_search | identifier | 25 | 0.800 | 80% | 80% | 80% | 129.8 |
| semantic_search | natural_language | 55 | 0.395 | 33% | 44% | 45% | 300.9 |
| semantic_search | short_phrase | 9 | 0.475 | 44% | 44% | 44% | 264.5 |
| get_ranked_context_no_semantic | identifier | 25 | 0.800 | 80% | 80% | 80% | 22.5 |
| get_ranked_context_no_semantic | natural_language | 55 | 0.365 | 27% | 42% | 49% | 35.9 |
| get_ranked_context_no_semantic | short_phrase | 9 | 0.423 | 22% | 56% | 67% | 33.5 |
| get_ranked_context | identifier | 25 | 0.800 | 80% | 80% | 80% | 22.9 |
| get_ranked_context | natural_language | 55 | 0.467 | 38% | 51% | 55% | 148.6 |
| get_ranked_context | short_phrase | 9 | 0.556 | 33% | 89% | 89% | 133.5 |
...
| natural_language | +0.102 | +11% | +9% | +5% |
| short_phrase | +0.133 | +11% | +33% | +22% |
...
| semantic_search | find where a symbol is defined in a file | 8 | find_symbol (crates/codelens-engine/src/symbols/mod.rs) |
| semantic_search | search code by natural language query | miss | is_natural_language_query (crates/codelens-mcp/src/tools/symbols.rs) |
...
| semantic_search | filter out standard library noise from call graph | 6 | supports_import_graph (crates/codelens-engine/src/import_graph/mod.rs) |
...
| semantic_search | calculate modularity score for graph partitioning | 9 | empty_graph_returns_empty (crates/codelens-engine/src/community.rs) |
...
| semantic_search | handle semantic search tool request | 7 | semantic_code_review (crates/codelens-mcp/src/tools/reports/impact_reports.rs) |
...
| semantic_search | get current timestamp in milliseconds | 9 | matches_scope (crates/codelens-mcp/src/artifact_store.rs) |
...
| get_ranked_context | search code by natural language query | miss | is_natural_language_query (crates/codelens-mcp/src/tools/symbols.rs) |
| get_ranked_context | read input from stdin line by line | 9 | read_file (crates/codelens-engine/src/file_ops/reader.rs) |
| get_ranked_context | parse source code into an AST | 11 | slice_source (crates/codelens-engine/src/symbols/parser.rs) |
...
| get_ranked_context | parse incoming MCP tool call JSON | 4 | write_mcp_json (install.sh) |
