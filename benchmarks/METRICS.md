...
| **onboard_project**    | <5s    | historical snapshot | fastembed override 세션 | 현재 기본 모델 기준 재측정 필요 |

### 병목 분석

> 이 섹션의 `45s`, `BGE-small`, `248MB` 수치는 2026-03-30 시점의 fastembed override 세션 스냅샷이다.
> 현재 기본 모델은 bundled `MiniLM-L12-CodeSearchNet-INT8`이며, 최신 런타임 수치는 `benchmarks/embedding-runtime.py`로 다시 측정해야 한다.
...
### 최신 로컬 런타임 스냅샷 (2026-04-02)

`python3 benchmarks/embedding-runtime.py . --output benchmarks/embedding-runtime-results.json`

```
모델: MiniLM-L12-CodeSearchNet-INT8
인덱스 상태(사전): 1,272 symbols already indexed
index_embeddings: 6,669.76ms → 1,679 symbols
semantic_search warm avg: 204.26ms (3 runs, 5 results each)
get_ranked_context warm avg: 219.41ms (3 runs, 15 results each)
onboard_project ready path: 64.78ms
semantic ready path: 기존 인덱스 재사용, 모델 로딩 없이 loaded=false
```

### 최신 로컬 품질 스냅샷 (2026-04-02)

`python3 benchmarks/embedding-quality.py . --output benchmarks/embedding-quality-results.json --markdown-output benchmarks/embedding-quality-summary.md`
