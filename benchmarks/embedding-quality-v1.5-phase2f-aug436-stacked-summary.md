...
|---|---:|---:|---:|---:|---:|
| semantic_search | 0.047 | 4% | 5% | 5% | 229.6 |
| get_ranked_context_no_semantic | 0.045 | 4% | 5% | 6% | 31.0 |
| get_ranked_context | 0.051 | 4% | 6% | 6% | 112.3 |

## Query Type Breakdown

| Method | Query type | Count | MRR | Acc@1 | Acc@3 | Acc@5 | Avg ms |
|---|---|---:|---:|---:|---:|---:|---:|
| semantic_search | identifier | 83 | 0.096 | 10% | 10% | 10% | 128.2 |
| semantic_search | natural_language | 299 | 0.037 | 3% | 4% | 4% | 259.7 |
| semantic_search | short_phrase | 54 | 0.021 | 2% | 2% | 2% | 218.3 |
| get_ranked_context_no_semantic | identifier | 83 | 0.096 | 10% | 10% | 10% | 22.4 |
| get_ranked_context_no_semantic | natural_language | 299 | 0.038 | 3% | 5% | 5% | 33.5 |
| get_ranked_context_no_semantic | short_phrase | 54 | 0.005 | 0% | 0% | 2% | 30.3 |
| get_ranked_context | identifier | 83 | 0.096 | 10% | 10% | 10% | 22.5 |
| get_ranked_context | natural_language | 299 | 0.043 | 3% | 5% | 5% | 134.4 |
| get_ranked_context | short_phrase | 54 | 0.028 | 2% | 4% | 4% | 128.7 |
...
| semantic_search | get project structure and key files on first load | 8 | get_project_structure (crates/codelens-mcp/src/tools/symbols.rs) |
...
| semantic_search | filter out standard library noise from call graph | miss | empty_graph_returns_empty (crates/codelens-engine/src/community.rs) |
...
| semantic_search | search for a symbol by name | miss | find_symbol (crates/codelens-engine/src/symbols/mod.rs) |
...
| semantic_search | find all word matches in a single file | miss | find_word_matches_in_files (crates/codelens-engine/src/rename.rs) |
...
| semantic_search | create a subscription checkout session | miss | session_preset_and_budget (crates/codelens-mcp/src/server/session.rs) |
...
| semantic_search | export data to CSV file from a table | miss | visible_axes_from_tools (crates/codelens-mcp/src/server/router.rs) |
...
| semantic_search | check user subscription plan and permissions | miss | delete (crates/codelens-engine/tests/fixtures/sample_project/src/models.ts) |
...
| semantic_search | run background job with a queue processor | miss | record_analysis_job_cancelled (crates/codelens-mcp/src/telemetry.rs) |
...
| semantic_search | run Celery background task asynchronously | miss | stage2_task_finetune (scripts/finetune/finetune_v2.py) |
...
| semantic_search | run Django database migration for a new field | miss | new (crates/codelens-engine/src/symbols/mod.rs) |
...
| semantic_search | stream large file response without loading into memory | miss | annotate_initialize_response (crates/codelens-mcp/src/server/transport_http_support.rs) |
...
| semantic_search | summarize a long text using an LLM | miss | analysis_summary_payload (crates/codelens-mcp/src/resource_analysis.rs) |
...
| semantic_search | parse JSON request body into a struct | miss | parse_lsp_args (crates/codelens-mcp/src/tools/mod.rs) |
...
| semantic_search | implement the Display trait for a custom type | miss | detects_unresolved_type (crates/codelens-engine/src/auto_import.rs) |
...
| semantic_search | read and write a TOML configuration file | miss | telemetry_writer_appends_multiple_events_in_order (crates/codelens-mcp/src/telemetry.rs) |
...
| semantic_search | check if two byte slices are equal in constant time | miss | embedding_to_bytes (crates/codelens-engine/src/embedding/mod.rs) |
...
| semantic_search | write a benchmark test with criterion | miss | make_project_with_source (crates/codelens-engine/src/embedding/mod.rs) |
...
| semantic_search | JWT token generation | miss | tokenizer (scripts/finetune/finetune_distill.py) |
...
| semantic_search | CSV row parser utility | miss | parser (crates/codelens-engine/src/symbols/mod.rs) |
...
| get_ranked_context_no_semantic | find where a symbol is defined in a file | miss | find_symbol_range (crates/codelens-engine/src/symbols/mod.rs) |
...
| get_ranked_context_no_semantic | move code to a different file | miss | test_file_path_to_module_js (crates/codelens-engine/src/move_symbol.rs) |
...
| get_ranked_context_no_semantic | validate that a new name is a valid identifier | miss | validate_identifier (crates/codelens-engine/src/rename.rs) |
...
| get_ranked_context_no_semantic | normalize file path relative to project root | miss | normalize_path (crates/codelens-engine/src/project.rs) |
...
| get_ranked_context_no_semantic | classify what kind of symbol this is | 5 | make_symbol_id (crates/codelens-engine/src/symbols/types.rs) |
...
| get_ranked_context_no_semantic | paginate database query results | miss | search (crates/codelens-engine/src/embedding/mod.rs) |
...
| get_ranked_context_no_semantic | run a database transaction with rollback on error | miss | with_transaction_auto_rollback_on_error (crates/codelens-engine/src/db/tests.rs) |
...
| get_ranked_context_no_semantic | background task queue | miss | AnalysisWorkerQueue (crates/codelens-mcp/src/analysis_queue.rs) |
...
| get_ranked_context | get project structure and key files on first load | 12 | get_project_structure (crates/codelens-engine/src/symbols/mod.rs) |
| get_ranked_context | watch filesystem for file changes | miss | FileWatcher (crates/codelens-engine/src/watcher.rs) |
| get_ranked_context | skip comments and string literals during search | miss | search (crates/codelens-engine/src/embedding/mod.rs) |
...
| get_ranked_context | find all word matches in a single file | miss | find_word_matches_in_files (crates/codelens-engine/src/rename.rs) |
...
| get_ranked_context | handle user authentication with JWT tokens | miss | GH_TOKEN (.github/workflows/release.yml) |
| get_ranked_context | create a custom React hook for fetching data | miss | set_created_at_for_test (crates/codelens-mcp/src/artifact_store.rs) |
...
| get_ranked_context | paginate database query results | miss | result (crates/codelens-mcp/src/protocol.rs) |
...
| get_ranked_context | manage global state with Zustand store | miss | store (crates/codelens-mcp/src/artifact_store.rs) |
...
| get_ranked_context | cache API responses in Redis | miss | cached_query (crates/codelens-engine/src/symbols/parser.rs) |
...
| get_ranked_context | intercept Axios requests to add auth headers | miss | request_headers (benchmarks/harness/harness_runner_common.py) |
| get_ranked_context | export data to CSV file from a table | miss | migrate (crates/codelens-engine/src/db/mod.rs) |
...
| get_ranked_context | parse markdown content to HTML | miss | render_markdown (benchmarks/paper-benchmark.py) |
| get_ranked_context | map tRPC router endpoints to handlers | miss | mcp_post_handler (crates/codelens-mcp/src/server/transport_http.rs) |
...
| get_ranked_context | sanitize user-generated HTML content | miss | sanitize_section_name (crates/codelens-mcp/src/artifact_store.rs) |
...
| get_ranked_context | create a serverless API route to handle POST requests | miss | mcp_post_handler (crates/codelens-mcp/src/server/transport_http.rs) |
...
| get_ranked_context | define a FastAPI route with path and query parameters | miss | router (crates/codelens-mcp/src/server/mod.rs) |
| get_ranked_context | create a Pydantic model for request body validation | miss | body_lines (scripts/finetune/finetune_v2.py) |
| get_ranked_context | run Celery background task asynchronously | miss | codex_home_tempdir (benchmarks/harness/codex-task-runner.py) |
| get_ranked_context | connect to database using SQLAlchemy async session | miss | session (crates/codelens-engine/src/lsp/mod.rs) |
...
| get_ranked_context | parse CSV file into a list of dicts | miss | extract_csharp_imports (crates/codelens-engine/src/import_graph/parsers.rs) |
...
| get_ranked_context | define GraphQL mutation to create a user | miss | build_graph_from_db (crates/codelens-engine/src/import_graph/mod.rs) |
| get_ranked_context | run a subprocess command and capture output | miss | result (benchmarks/benchmark_runtime_common.py) |
...
| get_ranked_context | preprocess text for NLP pipeline | miss | text (scripts/finetune/finetune_distill.py) |
...
| get_ranked_context | extract features from raw dataset for training | miss | raw_name (scripts/finetune/build_runtime_training_pipeline.py) |
...
| get_ranked_context | verify an HMAC signature on an incoming webhook | miss | signature (scripts/finetune/convert_csn_codelens_v2.py) |
...
| get_ranked_context | log a request and its response time as a middleware | miss | send_request (crates/codelens-engine/src/lsp/session.rs) |
...
| get_ranked_context | track model training metrics per epoch | miss | metrics_config (crates/codelens-mcp/src/tools/session/mod.rs) |
...
| get_ranked_context | apply database migrations using goose | miss | migrate (crates/codelens-engine/src/db/mod.rs) |
...
| get_ranked_context | serialize a struct to a map for dynamic querying | miss | build_type_map (crates/codelens-engine/src/type_hierarchy.rs) |
...
| get_ranked_context | rate limit requests using a token bucket algorithm | miss | buckets (scripts/finetune/build_runtime_training_pipeline.py) |
| get_ranked_context | upload a file to GCS bucket | miss | bucket (scripts/finetune/build_repo_adversarial_dataset.py) |
...
| get_ranked_context | create a Spring JPA repository interface | miss | JAVA (scripts/finetune/generate_curated_1k.py) |
...
| get_ranked_context | create a Spring Batch job for data processing | miss | batch (scripts/finetune/train_v6_internet_only.py) |
...
| get_ranked_context | publish a message to a RabbitMQ exchange | miss | send_message (crates/codelens-engine/src/lsp/protocol.rs) |
...
| get_ranked_context | build a CLI argument parser with clap derive | miss | parse_symbols (crates/codelens-engine/src/symbols/parser.rs) |
...
| get_ranked_context | verify HMAC-SHA256 signature in Rust | miss | resolve_rust_module (crates/codelens-engine/src/import_graph/resolvers.rs) |
...
| get_ranked_context | implement retry logic with a backoff strategy | miss | retry_analysis_job (crates/codelens-mcp/src/tools/report_jobs.rs) |
...
| get_ranked_context | handle a timeout on an async future | miss | timeout_secs (crates/codelens-mcp/src/server/session.rs) |
...
| get_ranked_context | serialize an ActiveRecord model to JSON with JBuilder | miss | json_path (benchmarks/token-efficiency.py) |
...
| get_ranked_context | create a Laravel Eloquent model with relationships | miss | create (crates/codelens-mcp/src/server/session.rs) |
...
| get_ranked_context | define a Laravel event and listener pair | miss | FileEvent (crates/codelens-engine/src/vfs.rs) |
| get_ranked_context | ApiResource | miss |  |
| get_ranked_context | ServiceProvider | miss |  |
| get_ranked_context | query database using Laravel Query Builder with joins | miss | all_with_embeddings (crates/codelens-engine/src/embedding_store.rs) |
...
| get_ranked_context | Redis cache client setup | miss | QUERY_CACHE (crates/codelens-engine/src/symbols/parser.rs) |
...
| get_ranked_context | user session persistence hook | miss | session (crates/codelens-mcp/src/tools/mod.rs) |
...
| get_ranked_context | Pydantic settings config | miss | LanguageConfig (crates/codelens-engine/src/lang_config.rs) |
...
| get_ranked_context | connection pool initialization | miss | pool (scripts/finetune/build_runtime_training_pipeline.py) |
