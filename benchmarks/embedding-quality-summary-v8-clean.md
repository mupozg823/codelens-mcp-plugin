...
|---|---:|---:|---:|---:|---:|
| semantic_search | 0.616 | 54% | 69% | 71% | 115.8 |
| get_ranked_context_no_semantic | 0.561 | 48% | 60% | 66% | 21.9 |
| get_ranked_context | 0.613 | 52% | 67% | 71% | 91.8 |

## Query Type Breakdown

| Method | Query type | Count | MRR | Acc@1 | Acc@3 | Acc@5 | Avg ms |
|---|---|---:|---:|---:|---:|---:|---:|
| semantic_search | identifier | 25 | 0.853 | 84% | 88% | 88% | 114.2 |
| semantic_search | natural_language | 55 | 0.509 | 42% | 58% | 62% | 116.5 |
| semantic_search | short_phrase | 9 | 0.611 | 44% | 78% | 78% | 116.2 |
| get_ranked_context_no_semantic | identifier | 25 | 1.000 | 100% | 100% | 100% | 19.3 |
| get_ranked_context_no_semantic | natural_language | 55 | 0.374 | 27% | 42% | 49% | 23.2 |
| get_ranked_context_no_semantic | short_phrase | 9 | 0.484 | 33% | 56% | 78% | 21.4 |
| get_ranked_context | identifier | 25 | 1.000 | 100% | 100% | 100% | 19.1 |
| get_ranked_context | natural_language | 55 | 0.457 | 35% | 51% | 56% | 120.4 |
| get_ranked_context | short_phrase | 9 | 0.492 | 22% | 78% | 78% | 119.2 |
...
| natural_language | +0.083 | +7% | +9% | +7% |
| short_phrase | +0.007 | -11% | +22% | +0% |

## Misses

| Method | Query | Rank | Top candidate |
|---|---|---:|---|
| semantic_search | apply text edits to multiple files | miss | collect_file_scope_edits (crates/codelens-core/src/rename.rs) |
| semantic_search | start an HTTP server with routes | miss | build_router (crates/codelens-mcp/src/server/transport_http.rs) |
| semantic_search | read input from stdin line by line | miss | find_import_insert_line (crates/codelens-core/src/auto_import.rs) |
| semantic_search | build embedding vectors for all symbols | miss | all_with_embeddings (crates/codelens-core/src/embedding.rs) |
| semantic_search | find near-duplicate code in the codebase | 6 | find_similar_code (crates/codelens-core/src/embedding.rs) |
...
| semantic_search | watch filesystem for file changes | miss | change_signature (crates/codelens-core/src/change_signature.rs) |
| semantic_search | extract lines of code into a new function | miss | replace_lines (crates/codelens-core/src/file_ops/writer.rs) |
| semantic_search | skip comments and string literals during search | miss | search_symbols_hybrid (crates/codelens-core/src/search.rs) |
| semantic_search | route an incoming tool request to the right handler | 4 | find_misplaced_code_handler (crates/codelens-mcp/src/dispatch.rs) |
| semantic_search | how to build embedding text from a symbol | miss | get_embedding (crates/codelens-core/src/embedding_store.rs) |
| semantic_search | AppState | miss | SessionState (crates/codelens-mcp/src/server/session.rs) |
| semantic_search | find all functions that call a given function | 5 | get_callers (crates/codelens-core/src/call_graph.rs) |
...
| semantic_search | measure density of internal edges in a cluster | miss | resolve_call_edges (crates/codelens-core/src/call_graph.rs) |
| semantic_search | register sqlite vector extension for similarity search | 8 | find_similar_code (crates/codelens-core/src/embedding.rs) |
...
| semantic_search | SymbolIndex | miss | EmbeddingIndexInfo (crates/codelens-core/src/embedding.rs) |
...
| semantic_search | MutationFailureKind | miss | evaluate_mutation_gate (crates/codelens-mcp/src/mutation_gate.rs) |
...
| semantic_search | select the most relevant symbols for a query | miss | search (crates/codelens-core/src/embedding.rs) |
| semantic_search | find similar code snippets using embeddings | 6 | find_similar_code (crates/codelens-core/src/embedding.rs) |
| semantic_search | classify what kind of symbol this is | miss | classify_symbol (crates/codelens-core/src/embedding.rs) |
| semantic_search | get current timestamp in milliseconds | miss | get_fresh_file_by_mtime (crates/codelens-core/src/db/ops.rs) |
| semantic_search | parse incoming MCP tool call JSON | miss | get_current_config (crates/codelens-mcp/src/tools/filesystem.rs) |
...
| get_ranked_context | rename a variable or function across the project | 7 | get_or_build (crates/codelens-core/src/import_graph/mod.rs) |
| get_ranked_context | apply text edits to multiple files | 11 | collect_file_scope_edits (crates/codelens-core/src/rename.rs) |
...
| get_ranked_context | start an HTTP server with routes | 15 | start_http_daemon (benchmarks/benchmark_runtime_common.py) |
| get_ranked_context | read input from stdin line by line | miss | first_line (scripts/finetune/build_runtime_training_pipeline.py) |
...
| get_ranked_context | find near-duplicate code in the codebase | 7 | find_similar_code (crates/codelens-core/src/embedding.rs) |
| get_ranked_context | get project structure and key files on first load | miss | key_files (benchmarks/benchmark_project_context.py) |
| get_ranked_context | extract lines of code into a new function | 22 | query_lines (scripts/finetune/extract_major_langs.py) |
| get_ranked_context | skip comments and string literals during search | miss | search_symbols_hybrid (crates/codelens-core/src/search.rs) |
| get_ranked_context | route an incoming tool request to the right handler | 4 | find_misplaced_code_handler (crates/codelens-mcp/src/dispatch.rs) |
| get_ranked_context | how to build embedding text from a symbol | 22 | build_symbol_text (scripts/finetune/collect_training_data.py) |
| get_ranked_context | mutation gate preflight check before editing | 5 | record_mutation_preflight_checked (crates/codelens-mcp/src/telemetry.rs) |
| get_ranked_context | find all functions that call a given function | 8 | extract_name_javascript (scripts/finetune/convert_csn_codelens_v2.py) |
| get_ranked_context | resolve which file a called function belongs to | miss | extract_name_javascript (scripts/finetune/convert_csn_codelens_v2.py) |
...
| get_ranked_context | register sqlite vector extension for similarity search | 7 | find_similar_code (crates/codelens-core/src/embedding.rs) |
...
| get_ranked_context | build embedding index for all project symbols | miss | inspect_existing_index (crates/codelens-core/src/embedding.rs) |
...
| get_ranked_context | how does the embedding engine initialize the model | 15 | inspect_existing_index (crates/codelens-core/src/embedding.rs) |
| get_ranked_context | select the most relevant symbols for a query | 16 | get_important_symbols (scripts/finetune/augment_dataset.py) |
| get_ranked_context | find similar code snippets using embeddings | 7 | find_similar_code (crates/codelens-core/src/embedding.rs) |
| get_ranked_context | classify what kind of symbol this is | 25 | classify_symbol (crates/codelens-core/src/embedding.rs) |
| get_ranked_context | get current timestamp in milliseconds | miss | get_fresh_file_by_mtime (crates/codelens-core/src/db/ops.rs) |
| get_ranked_context | parse incoming MCP tool call JSON | 9 | tool_calls (benchmarks/harness/harness_eval_common.py) |
